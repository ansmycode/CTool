...
本次表内数量字段规则是 CTool 的新增启发式规则，仅有合成测试，尚未经真实游戏验收。当前实现的记录边界检查不代表引擎整体能力的上限；游戏自身初始化或其他业务操作可能建立数量记录。

CTool 的 DLL 接收 `inventorywrite`，按数据库类型、表、记录、字段定位数字单元格，再检查预期旧值并写入新值。`writeNumber` 校验记录范围、数字字段、可写内存和指针状态，执行比较交换后重新解析并回读。

映射层导出 `quantityBinding`，UI 只给出分类和物品 ID，主进程实时重建 binding 后才写入。表编号与字段编号来自当前游戏目录，不固定为某个测试游戏的值。当前没有自动扩容运行时记录的逻辑。

名称相似不足以证明字段就是玩家持有数量。不同游戏的数量映射仍需结合实际获得、消耗及读档行为验证，不能直接手工扩容 `Vector`。
...
| 尚未支持 | 当目标布局没有该运行时记录时，当前写入会拒绝。需要进一步定位并验证游戏建立数量记录的流程，不能伪造/扩容内存 Vector。 |
...
1. 在游戏内正常获得一件物品，对比获得前后的数据库记录和数量，确认游戏菜单实际出现。
2. 在 CTool DEV 数据库页中定位同一坐标，记录字段值；验证它不是仅凭“所持个数”表的相同行号猜出的目标。
3. 在游戏内正常获得、设为 0、重新获得并读档；确认绑定记录与字段始终稳定，观察菜单刷新。
4. 若直接数值槽写入不能得到预期结果，对游戏正常获得物品时的写入设置断点，检查记录初始化与相关字段更新流程；不提前假设需要新增整行记录。
