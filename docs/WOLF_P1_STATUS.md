...
当前进展（2026-09-17）：已支持数据库双向 RPC、用户验收的金币修改与已有数量记录修改；库存写前由主进程重新解析语义绑定，DLL 使用通用 writeNumber。缺失记录创建及所有注册物品覆盖尚未完成；新增表内数量字段规则待实测。当前实现见 [代码导读](WOLF_CODE_WALKTHROUGH.md) 与 [映射说明](WOLF_MAPPING.md)。本文以下保留 P0/P1 首轮状态与历史验证范围，不代表当前功能上限。
