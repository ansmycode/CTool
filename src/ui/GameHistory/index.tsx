...
import { Image, Space, Button, notification, Popconfirm } from "antd";

import "./index.css";

interface Props {
  historyLaunchGame: (info: any) => void;
}

const GameHistory: React.FC<Props> = ({ historyLaunchGame }) => {
  const [gameHistory, setGameHistory] = useState<any[]>([]);
  const [api, contextHolder] = notification.useNotification();

  const getGameHistory = async () => {
    const _gameHistory: any = await window.electronAPI.readGameHistory();
  ...
  const handleDeleteHistory = async (gamePath: string) => {
    const result: any = await window.electronAPI.deleteGameHistory(gamePath);
    if (result.success) {
      api.success({ message: "删除成功", description: "" });
      getGameHistory();
    } else {
      api.error({ message: "错误", description: result?.message });
    }
  };
          ...
          <div key={item.gamePath} className="game-history-item">
            <Space size="middle" className="game-history-item-space">
              <Image
                ...
                className="game-history-item-img"
                preview={false}
              />
              <Space
                direction="vertical"
                size="middle"
                className="game-history-item-info"
              >
                <div>游戏名称: {item.title}</div>
                <div>游戏路径: {item.gamePath}</div>
                ...
                <Space>
                  ...
                  <Popconfirm
                    ...
                    onConfirm={() => handleDeleteHistory(item.gamePath)}
                    okText="Yes"
                    cancelText="No"
                  >
                    <Button danger>从记录中删除</Button>
                  </Popconfirm>
                </Space>
              </Space>
            </Space>
