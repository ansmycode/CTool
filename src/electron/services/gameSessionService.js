  ...
  return {
    ...
    async launch(exePath) {
      if (current && !current.finalized)
        throw new Error("已有游戏会话，请先关闭当前游戏");
      const entry = {
        snapshot: {
      ...
      try {
        ...
        if (!game?.supported)
          throw new Error(game?.supportMessage || "不支持的游戏");
          ...
          emit(event) {
            ...
            if (event.type === "spawned") {
              ...
              try {
                ...
                /* History must not invalidate a running game. */
              }
            } else if (event.type === "injected") {
              if (entry.snapshot.state === "connecting")
                update(entry, {
                  state: "initializing",
                  message: "DLL 已加载，等待握手",
                });
            } else if (event.type === "connected") {
              update(entry, {
                state: "degraded",
                capabilities: [],
                databaseReadOnly: !!event.databaseReadOnly,
                goldWritable: !!event.goldWritable,
                inventoryWritable: !!event.inventoryWritable,
                message: event.message,
              });
            } else if (event.type === "telemetry") {
              if (
                entry.snapshot.state === "degraded" &&
                entry.snapshot.processState === "running"
              )
    ...
    markMvmzReady() {
      if (
        !current ||
        current.finalized ||
        !["MV", "MZ"].includes(current.snapshot.game?.engine) ||
        current.snapshot.processState !== "running"
      )
        return;
      update(current, {
        state: "ready",
        capabilities: mvmzCapabilities,
        message: "游戏已就绪",
      });
    },
    async readDatabase(sessionId, request) {
      ...
      if (
        ...
        !entry.snapshot.databaseReadOnly ||
        !entry.driver.readDatabase
      )
    ...
    selectGoldSource(sessionId, target) {
      if (
        ...
        !current.snapshot.databaseReadOnly ||
        !current.driver.selectGoldSource
      )
        throw new Error("数据库会话无效或未就绪");
      current.driver.selectGoldSource(target);
      update(current, {
        telemetry: {
          gold: { status: "unavailable", reason: "gold_source_switching" },
        },
      });
    },
    refreshTelemetry(sessionId) {
      if(!current||current.finalized||current.snapshot.sessionId!==sessionId||!current.snapshot.databaseReadOnly)return;
      current.driver.refreshTelemetry?.();
    },
    async setGold(sessionId, value, expectation) {
      ...
      if (
        ...
        entry.finishing ||
        entry.snapshot.sessionId !== sessionId ||
        !entry.snapshot.goldWritable ||
        !entry.snapshot.databaseReadOnly ||
        !entry.driver.setGold
      )
      ...
      if (
        ...
        throw new Error("会话已变化，修改结果未确认");
    },
    async setInventoryCount(sessionId, target, expected, value) {
      const entry = current;
      if (
        !entry ||
        entry.finalized ||
        entry.finishing ||
        entry.snapshot.sessionId !== sessionId ||
        !entry.snapshot.inventoryWritable ||
        !entry.snapshot.databaseReadOnly ||
        !entry.driver.setInventoryCount
      )
        throw new Error("背包修改会话无效或未就绪");
      await entry.driver.setInventoryCount(target, expected, value);
      if (
        current !== entry ||
        entry.finalized ||
        entry.finishing ||
