...
export function createGameSessionService({ detect, createDriver, publish, saveHistory = () => {} }) {
  ...
  return {
    ...
    async setInventoryCount(sessionId,target,expected,value) {
      const entry=current;
      if(!entry||entry.finalized||entry.finishing||entry.snapshot.sessionId!==sessionId||!entry.snapshot.inventoryWritable||!entry.snapshot.databaseReadOnly||!entry.driver.setInventoryCount)
        throw new Error("背包修改会话无效或未就绪");
      await entry.driver.setInventoryCount(target,expected,value);
      if(current!==entry||entry.finalized||entry.finishing||!entry.snapshot.inventoryWritable)throw new Error("会话已变化，修改结果未确认");
    },
